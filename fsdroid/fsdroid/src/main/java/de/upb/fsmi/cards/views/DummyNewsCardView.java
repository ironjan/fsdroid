...
public class DummyNewsCardView extends RelativeLayout {
	...
	public void bind(Item pRssItem) {
		...
		newsDate.setText(SimpleDateFormat.getDateTimeInstance().format(
				pRssItem.getPubDate()));
		displayItemContent(pRssItem.getContent());

	}

	private void displayItemContent(Content pContent) {
		//bug in WebView.loadData(data, mimeType, encoding) which fucks up utf-8
		newsText.loadDataWithBaseURL(null, pContent.getValue(), "text/html", "utf-8",null);
	}
